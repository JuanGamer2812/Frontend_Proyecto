...
export class InsertarProveedor implements OnInit {
  ...
  // Archivos/URLs de características dinámicas
  caracteristicasFiles: Record<string, File> = {};
  caracteristicasUrls: Record<string, string> = {};
  caracteristicasModo: Record<string, 'file' | 'url'> = {};
  ...
  // Detectar si característica requiere archivo/URL (si contiene "PDF" o "URL" en nombre/descripción)
  esCaracteristicaFileUrl(car: any): boolean {
    const texto = `${car?.nombre_caracteristica || ''} ${car?.descripcion || ''} ${car?.nombre || ''}`.toUpperCase();
    return texto.includes('PDF') || texto.includes('URL');
  }

  // Alternar modo file/url para característica
  toggleCaracteristicaModo(carId: string, modo: 'file' | 'url'): void {
    this.caracteristicasModo[carId] = modo;
    if (modo === 'file') {
      delete this.caracteristicasUrls[carId];
    } else {
      delete this.caracteristicasFiles[carId];
    }
  }

  // Manejar selección de archivo para característica
  onCaracteristicaFileChange(event: any, carId: string): void {
  ...
  // Manejar URL para característica
  onCaracteristicaUrlChange(carId: string, url: string): void {
    this.caracteristicasUrls[carId] = url;
    const control = this.formProveedor.get('caracteristicas')?.get(carId);
    if (control) control.setValue(url);
  }
  ...
  enviarProveedor(): void {
    ...
    if (Object.keys(this.caracteristicasUrls).length > 0) {
      formData.append('caracteristicas_urls', JSON.stringify(this.caracteristicasUrls));
    }
  ...
  onReset(): void {
    ...
    this.caracteristicasFiles = {};
    this.caracteristicasUrls = {};
    this.caracteristicasModo = {};
    this.mensajeExito = '';
    this.mensajeError = '';
    this.buildCaracteristicasForm([]);
  }
