  ...
  async onSubmit(): Promise<void> {
    ...
    this.apiService.createReserva(reservasPayload).subscribe({
      next: async (resReservas) => {
        ...
        // Guardar solo las características modificadas (si las hay) usando idEvento devuelto
        ...
        this.proveedorCaracteristicasEditable.forEach((lista, idx) => {
          ...
          (lista || []).forEach((c: any) => {
            ...
            let changed = false;
            if (c.tipo === 'numero') changed = Number(origVal) !== Number(val);
            else if (c.tipo === 'booleano' || c.tipo === 'boolean') changed = !!origVal !== !!val;
            else changed = String(origVal ?? '') !== String(val ?? '');
            if (!changed) return;
            const item: any = { id_evento: idEvento, id_proveedor: idProveedor, id_caracteristica: Number(c.id_caracteristica) };
            if (c.tipo === 'numero') item.valor_numero = Number(val);
            else if (c.tipo === 'booleano' || c.tipo === 'boolean') item.valor_booleano = !!val;
            else if (c.tipo === 'json') item.valor_json = val;
            else item.valor_texto = String(val);
            caracteristicasPayload.push(item);
          });
        });

        if (caracteristicasPayload.length > 0) {
          console.log('Crear evento_proveedor_caracteristicas payload:', caracteristicasPayload);
          this.apiService.crearEventoProveedorCaracteristicas(caracteristicasPayload).subscribe({ next: (resC) => { console.log('crearEventoProveedorCaracteristicas response:', resC); }, error: (errC) => { console.error('Error crearEventoProveedorCaracteristicas', errC); } });
        }
