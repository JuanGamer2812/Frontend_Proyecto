...
export class PasswordResetService {
  ...
  changePassword(currentPassword: string, newPassword: string): Observable<any> {
    return this.http.post(`${environment.apiUrl}/password/change`, { currentPassword, newPassword });
  }
  ...
  validateToken(token: string): Observable<any> {
    return this.http.post(`${environment.apiUrl}/password/validate-token`, { token });
  }

  resetPassword(token: string, newPassword: string): Observable<any> {
    return this.http.post(`${environment.apiUrl}/password/reset`, { token, newPassword });
  }
}
